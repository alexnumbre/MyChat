...
void Chat::signUp() {
	...
	User user = User(login, password, name) ;// создаем объект класса User
...
void Chat::userMenu() {
	char operation;
	std::cout << "Hi, " << currentUser_->GetUserName() << std::endl;// меню пользователя
	while (currentUser_) {
...
void Chat::loginMenu() {
	...
	do {
		...
		switch (operation) {
		case '1':
			login();//заходим на уже созданный аккаунт
			break;
		case '2':
			try {
				signUp();//регистрируем новый аккаунт
			}
			catch (const std::exception& e) {//ловим исключение, вызванное в  signup
	...
	} while (!currentUser_ && Working);//цикл работает, пока не зарегистрирован пользователь, и пока чат работает
}
